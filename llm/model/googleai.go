...
func queryGoogleAI(prompts []string, llm definitions.Model) ([]string, error) {
	...
	// Log selected model
	logger.Info("[GoogleAI] Using model: %s", llm.Model)
	...
	for i, prompt := range prompts {
		logger.Info("[GoogleAI] Sending prompt #%d: %s", i+1, prompt)
		...
		if err != nil {
			logger.Error("[GoogleAI] Failed to marshal response for prompt #%d: %v", i+1, err)
			return nil, err
		}
		logger.Info("[GoogleAI] Full response for prompt #%d: %s", i+1, string(respJSON))

		// Extract content from first candidate
		content := resp.Candidates[0].Content
		if content == nil || len(content.Parts) == 0 {
			logger.Error("[GoogleAI] No content parts in response for prompt #%d", i+1)
		...
		for _, part := range content.Parts {
			...
			case genai.Text:
				resultText += string(v)
			default:
				logger.Error("[GoogleAI] Unhandled response part type: %T", part)
			}
		}

		// Validate extracted text
		if resultText == "" {
			logger.Error("[GoogleAI] No text content extracted for prompt #%d", i+1)
